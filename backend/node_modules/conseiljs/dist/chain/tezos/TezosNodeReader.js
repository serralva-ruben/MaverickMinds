...
(function (TezosNodeReader) {
    ...
    function getBlockAtLevel(server, level, chainid = 'main') {
        return __awaiter(this, void 0, void 0, function* () {
            return performGetRequest(server, `chains/${chainid}/blocks/${level}`).then(json => { return json; });
        });
    }
    TezosNodeReader.getBlockAtLevel = getBlockAtLevel;
    ...
    function awaitOperationConfirmation(tezosNode, block, operationHash, limit = 10) {
        return __awaiter(this, void 0, void 0, function* () {
            let lastSeenBlock = block;
            if (lastSeenBlock < 0) {
                const head = yield TezosNodeReader.getBlockHead(tezosNode);
            ...
            while (lastSeenBlock < block + limit) {
                try {
                    const blockData = yield TezosNodeReader.getBlock(tezosNode, `${lastSeenBlock + 1}`);
                    ...
                    if (relevantOperation.length === 1) {
                        ...
                        yield new Promise(resolve => setTimeout(resolve, TezosConstants_1.TezosConstants.DefaultBlockTime));
                    }
                }
                catch (e) {
    ...
    TezosNodeReader.awaitOperationConfirmation = awaitOperationConfirmation;
